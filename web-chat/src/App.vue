  ...
  <main id="app">
    ...
    <section class="wrapper ai-window">
      ...
      <table v-for="a in answers" class="chat-window">
        ...
        <!-- <tr v-if="a.type == 'live-agent-info' && !liveAgentConnectedMessageShown">
          <div>Agent Adam gotowy do działania!</div>
        </tr> -->
...
const wsClient = io("http://localhost:3001");
...
export default {
  ...
  data: function() {
    return {
      ...
      config,
      connectedToLiveAgent: false,
      sessionId: undefined,
      liveAgentConnectedMessageShown: false
    };
  },
  watch: {
  ...
  methods: {
    submit() {
      if (this.connectedToLiveAgent) {
        console.log("submiting using wsClient");
        this.answers.push({
          result: { resolvedQuery: this.query, fulfillment: {} },
          type: "live-agent-info"
        });
        this.liveAgentConnectedMessageShown = true;

        wsClient.emit(this.sessionId, this.query);
        this.query = "";
        return;
      }
    ...
    handle(response) {
      ...
      this.connectedToLiveAgent = !!response.connectedToLiveAgent;
      if (response.connectedToLiveAgent) {
        console.log("connectedToLiveAgent, sessionId: ", response.sessionId);
        ...
        let self = this;
        wsClient.on(response.sessionId, function(agentResponse) {
          console.log("Response from liveagent: ", agentResponse);
          self.answers.push(wrapInResponseBody(agentResponse));
        });
      }
...
function wrapInResponseBody(message) {
  return {
    result: {
      //   resolvedQuery: message,
      fulfillment: {
        speech: message,
        messages: [{ type: 0, speech: message }]
      }
    },
